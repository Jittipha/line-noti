...
router.get(
  "/get/profile/:id",
  auth.verifyToken,
  personalController.Get_Profile
);
router.get("/getbyid/:id", personalController.getByID);
router.get("/line/disconnect/:id", personalController.Disconnect_line);
router.get("/get/upcoming/booking/:id", personalController.GetUpcoming_Booking);
router.get(
  "/check/already/:identificationNumber",
  personalController.Checkalready
);
router.get("/cancal/line/:id", personalController.cancelLine);
router.post(
  "/add/image",
  upload.single("file"),
  personalController.Upload_Image
);
...
router.post(
  "/check/phone",
  personalValidate.checkBody_CheckPhoneNumber,
  validator,
  personalController.CheckPhoneNumber
);
router.post("/check/emailandblood", personalController.CheckEmailAndBloodtype);
router.post("/adddata", personalController.adddata);
router.post(
  "/getall/myappoint/month",
  personalController.getAllMyappoint_inMonth
);
router.post(
  "/getall/myappoint/date",
  personalController.getAllMyappoint_inDate
);
router.post(
  "/update/password",
  personalValidate.checkBody_resetPassword,
  validator,
  personalController.resetPassword
);
router.post(
  "/change/password",
  personalValidate.checkBody_ChangePassword,
  validator,
  personalController.change_password
);
