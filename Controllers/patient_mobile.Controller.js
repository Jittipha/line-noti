...
exports.Disconnect_line = async (req, res) => {
  let PATIENT_ID = req.params.id;
  try {
    if (!PATIENT_ID) {
      return res.send({ error: true, message: "กรุณากรอกข้อมูลให้ครบ" });
    }
    const data = await personal.findOne({ PATIENT_ID });
    data.connectLine = "";
...
exports.getByID = async (req, res) => {
  ...
  try {
    ...
    if (queue_data.length != 0) {
      ...
      queue_data[0].AVG_WAITING_TIME =
        get_count_queue_in_date.length * queue_data[0].appoint.DURATION;
    }
    ...
    appointment
      ...
      .exec(async (err, data) => {
        ...
        for (let x = 0; x < data.length; x++) {
          ...
          if (
            Math.sign(
              service.cal_diff_date(real_date, data[x].APPOINT_DATE)
            ) === 0
          ) {
            if (starttime > HOUR) {
...
exports.Checkalready = async (req, res) => {
  ...
  try {
    ...
    if (data.length == 0) {
      console.log("working");
      const check_main_personal = await main_personal.findOne({
        ID_CARD: identificationNumber,
      });
      if (check_main_personal === null) {
        ...
        return res.send({
          error: false,
          message: "ผู้ใช้งานนี้มีบัญชีแล้ว (HIS)",
          data: {
            PATIENT_ID: check_main_personal._id,
            identificationNumber: check_main_personal.ID_CARD,
            hn: check_main_personal.HN,
            prefix: check_main_personal.PRE_NAME,
            firstName: check_main_personal.FIRST_NAME,
            lastName: check_main_personal.LAST_NAME,
            gender: check_main_personal.GENDER,
            dateofBirth: check_main_personal.BIRTHDATE,
            phone: phone,
          },
        });
      }
    } else {
      const data_patient = await main_personal.findOne({
        _id: data[0].PATIENT_ID,
      });
      let split = data_patient.MOBILE.split("");
      const phone = `${split[0]}${split[1]}x-xxx-${split[6]}${split[7]}${split[8]}${split[9]}`;
      return res.send({
        error: true,
        message: "มีผู้ใช้งานนี้อยู่ในระบบ",
        data: { _id: data[0]._id, PATIENT_ID: data_patient._id, phone: phone },
      });
    }
  } catch (err) {
    return res.status(400);
...
exports.CheckEmailAndBloodtype = async (req, res) => {
  ...
  try {
    ...
    return res.send({
      error: false,
      data: { email: emailFlag, bloodType: bloodTypeFlag },
    });
  } catch (err) {
    return res.status(400);
  }
  ...
  try {
    ...
    if (await bcrypt.compare(password, Personal.password)) {
      ...
      return res.send({
        ...
        message: message,
        data: {
          PATIENT_MOBILE_ID: Personal._id,
...
exports.logout = async (req, res) => {
  try {
    ...
    const data_patient = await personal.updateOne(
      { PATIENT_ID },
      { logoutFlag: "Y" }
    );
...
exports.adddata = async (req, res) => {
  ...
  try {
    ...
    const personal_data = await personal.create({
      PATIENT_ID,
      imageIDcard: hashedimageIDcard,
      identificationNumber,
      connectLine,
      accessTokenLine,
      imageIDcard,
...
exports.cancelLine = async (req, res) => {
  ...
  try {
    if (!_id) {
      return res.send({ error: true, message: "Please กรอกข้อมูลให้ครบ" });
    }
    const personal_data = personal.findOne({ _id }).exec(async (err, data) => {
  ...
  try {
    ...
    personal.findOne({ _id }).exec(async (err, data) => {
      ...
      return res.send({
        ...
        data: `${data_main.PRE_NAME} ${data_main.FIRST_NAME} ${data_main.LAST_NAME}`,
      });
    });
  } catch (err) {
    return res.status(400);
...
exports.updatedata = async (req, res) => {
  ...
  try {
    ...
    personal_data.identificationNumber =
      identificationNumber ?? personal_data.identificationNumber;
    ...
    personal_data.accessTokenLine =
      accessTokenLine ?? personal_data.accessTokenLine;
...
exports.update_main_data = async (req, res) => {
  ...
  try {
    ...
    data_main_patient.BIRTHDATE =
      BIRTHDATE_CONVERTED ?? data_main_patient.BIRTHDATE;
...
exports.deleteOne = async (req, res) => {
  ...
  try {
    ...
    const personal_data = await personal
      .deleteOne({ _id: id })
      .exec(async (err, data) => {
        if (err) return res.send({ error: true, data: err });
...
exports.getMyappoint_Upcoming = async (
  PATIENT_ID,
  startOfMonth,
  endOfMonth
) => {
...
exports.updatedata = async (req, res) => {
  ...
  try {
    ...
    mainpersonal_data.E_FIRST_NAME =
      E_FIRST_NAME ?? mainpersonal_data.E_FIRST_NAME;
    mainpersonal_data.E_LAST_NAME =
      E_LAST_NAME ?? mainpersonal_data.E_LAST_NAME;
  ...
  setInterval(async function () {
    try {
      ...
      if (queue_data.length != 0) {
        ...
        main_data_send.AVG_WAITING_TIME =
          get_count_queue_in_date.length * queue_data[0].appoint.DURATION;
