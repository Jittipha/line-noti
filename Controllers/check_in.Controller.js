...
const line_noti_controller = require("../Controllers/line_notify.Controller");
...
exports.add_queue = async (req, res) => {
  ...
  try {
    ...
    const data_depart = await depart
      .findOne({ _id: DEPART_ID })
      .select("_id departmentName");
    let queueNumber = "A333";
    const add_queue = await queue.create({
      PATIENT_ID,
      APPOINT_ID,
      DEPART_ID,
      DOCTOR,
    ...
    const add_queue_list = await queue_list.create({
      orNumber: "1",
      status: "W",
      DEPART_ID,
      QUEUE_ID: add_queue._id,
      APPOINT_ID,
    });
    line_noti_controller
      .sendMessageLine(
        "เลขคิวของท่านคือ " +
          queueNumber +
          " ห้องตรวจ : " +
          data_depart.departmentName,
        PATIENT_ID
      )
      .then((data) => {
        if (data == "success") {
          ...
          return res.send({
            error: false,
            message: "Created but don't send message",
            QUEUE_ID: add_queue._id,
          });
        }
      });
...
exports.getdata_queue_list = async (req, res) => {
  ...
  try {
    ...
    for (let x = 0; x < queue_list_data.length; x++) {
      const startOfday = moment(queue_list_data[x].createdAt)
        .startOf("day")
        .toDate();
      const endOfday = moment(queue_list_data[x].createdAt)
        .endOf("day")
        .toDate();
      ...
      queue_list_data[x].AVG_WAITING_TIME =
        get_count_queue_in_date.length * queue_list_data[0].appoint.DURATION;
    }
...
exports.delete_queue = async (req, res) => {
  ...
  try {
    ...
    if (
      delete_queue.deletedCount === 1 &&
      delete_queue_list.deletedCount === 1
    ) {
